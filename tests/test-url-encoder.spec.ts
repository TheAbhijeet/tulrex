...
test('test', async ({ page }) => {
    ...
    await page
        .getByRole('textbox', { name: 'Input Text / URL Component:' })
        .fill('https://djangocentral.com/');
    ...
    await expect(page.getByRole('textbox', { name: 'Output:' })).toHaveValue(
        'https%3A%2F%2Fdjangocentral.com%2F'
    );
    ...
    await page
        .getByRole('textbox', { name: 'Input Text / URL Component:' })
        .fill('https%3A%2F%2Fdjangocentral.com%2F');
    await page.getByRole('button', { name: 'Encode URI Component' }).click();
    await expect(page.getByLabel('Output:')).toContainText(
        'https%253A%252F%252Fdjangocentral.com%252F'
    );
