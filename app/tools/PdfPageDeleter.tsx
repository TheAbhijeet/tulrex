...
export default function PdfPageDeleter() {
    ...
    const loadPdfInfo = useCallback(async (file: File) => {
        setIsLoading(true); setError(''); setTotalPages(0); setPagesToDelete('');
        try {
        ...
        } catch (err: any) { setError(`Failed to load PDF info: ${err.message}`); console.error(err); setPdfFile(null);}
        finally { setIsLoading(false); }
    }, []);

    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const file = event.target.files?.[0];
        if (file && file.type === 'application/pdf') { setPdfFile(file); loadPdfInfo(file); }
        else { setPdfFile(null); setError(file ? 'Please select a valid PDF file.' : ''); setTotalPages(0); }
    };
    ...
    const parsePageNumbers = (input: string, maxPage: number): number[] | null => {
        ...
        for (const part of parts) {
            ...
            if (trimmedPart.includes('-')) {
                ...
                if (isNaN(start) || isNaN(end) || start < 1 || end > maxPage || start > end) return null; // Invalid range
    ...
    const handleDeleteAndSave = useCallback(async () => {
        ...
        if (!pagesToDelete.trim()) { setError('Please specify page numbers to delete.'); return; }
        ...
        if (indicesToRemove.length === totalPages) {
            setError('Cannot delete all pages.'); setIsProcessing(false); return;
        }
        ...
        try {
            ...
            setPdfFile(null); setTotalPages(0); setPagesToDelete('');
            if (fileInputRef.current) fileInputRef.current.value = '';

        } catch (err: any) { setError(`Failed to process PDF: ${err.message}`); console.error(err); }
    ...
    return (
         <div className="space-y-5 max-w-lg mx-auto">
             <div className="p-4 border border-dashed border-slate-600 rounded-md text-center bg-slate-800">
                 <label htmlFor="pdf-delete-upload" className="block text-sm font-medium text-slate-300 mb-2">Select PDF File:</label>
                  <Input ref={fileInputRef} id="pdf-delete-upload" type="file" accept="application/pdf" onChange={handleFileChange} disabled={isLoading || isProcessing} className="mx-auto block w-full max-w-sm text-sm file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-cyan-600 file:text-white hover:file:bg-cyan-700" />
             </div>
             {isLoading && <p className="text-center text-cyan-400 animate-pulse">Loading PDF info...</p>}
             ...
             {pdfFile && totalPages > 0 && !isLoading && (
                 <div className="space-y-3">
                    <p className="text-sm text-slate-300 text-center">Total Pages: {totalPages}</p>
                     <div>
                         <label htmlFor="pages-to-delete" className="block text-sm font-medium text-slate-300 mb-1">Pages to Delete (e.g., 1, 3-5, 8):</label>
                         <Input id="pages-to-delete" type="text" value={pagesToDelete} onChange={(e) => setPagesToDelete(e.target.value)} placeholder={`1-${totalPages}`} disabled={isProcessing}/>
                     </div>
                       <Button onClick={handleDeleteAndSave} disabled={isProcessing || !pagesToDelete.trim()} className="w-full mt-3 flex items-center justify-center gap-2">
